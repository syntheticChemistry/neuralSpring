# neuralSpring v11 → ToadStool / BarraCUDA Team Handoff

**Date:** February 22, 2026 (Session 42 — ToadStool sync + deep audit)
**From:** neuralSpring (ML validation & evolutionary computation biome)
**To:** ToadStool / BarraCUDA core team
**License:** AGPL-3.0-or-later
**Supersedes:** `archive/NEURALSPRING_V10_TOADSTOOL_BARRACUDA_HANDOFF_FEB22_2026.md`
...
Session 42 also synced neuralSpring to ToadStool HEAD `5437c170` (10 commits since
`d45fdfb3`). Build compiles cleanly with zero breaking changes. Four new bio-op
wrapper APIs are now available (`HillGateGpu`, `MultiObjFitnessGpu`, `PairwiseL2Gpu`,
`SwarmNnGpu`) and `cpu_conv_pool` was exposed for Springs. 19 new WGSL shaders
close CPU-only gaps in chi-squared, RK45, factorial, and cubic spline domains.
...
## Part 1b: ToadStool Evolution Since d45fdfb3

| Session | Commits | Key Changes |
|---------|---------|-------------|
| S39 | 3 | Spring shader absorption (7 bio + 11 HFB + 3 wetSpring), S-14/S-15/S-16 fixes, FlatTree, sparse_eigh |
| S40 | 1 | Richards PDE solver, moving window GPU stats |
| S41 | 4 | `cpu_conv_pool` pub, 6 f64 shader fixes, API exposure, stale doc archive |
| S42 | 2 | 19 new WGSL shaders (chi2, rk45, factorial, spline, etc.), BarraCUDA→BarraCuda doc rename |

### Newly Available Wrapper APIs

| API | Domain | Can Replace |
|-----|--------|-------------|
| `ops::bio::HillGateGpu` | Signal (021) | Local hill_gate.wgsl dispatch |
| `ops::bio::MultiObjFitnessGpu` | Directed evo (014) | Local multi_obj_fitness.wgsl dispatch |
| `ops::bio::PairwiseL2Gpu` | MODES (012) | Local pairwise_l2.wgsl dispatch |
| `ops::bio::SwarmNnGpu` | Swarm (015) | Local swarm_nn_forward.wgsl dispatch |
| `cpu_conv_pool::conv2d/max_pool2d` | LeNet-5 (Study 003) | Full conv+pool bC validation |
...
**Update:** `cpu_conv_pool` is now `pub` — resolved at CPU level for Springs;
GPU wiring to GpuExecutor is still pending.
