...
def main():
    ...
    try:
        ...
        print(f"  [PASS] Architecture validated (no data)")
        return 0
    ...
    print(f"  Architecture: Conv(1→6,5) → Pool → Conv(6→16,5) → Pool → "
          f"FC(400→120→84→10)")
    ...
    for epoch in range(n_epochs):
        ...
        scheduler.step()
        print(f"    Epoch {epoch+1:>2d}: train_acc={train_acc:.4f}, "
    ...
    print(f"  Paper reported: 99.05% (original LeNet-5)")

    if final_acc >= 0.985:
        print(f"  [PASS] Test accuracy ≥ 98.5%")
        total_passed += 1
    else:
    ...
    min_digit_acc = min(confusion[d][d] / confusion[d].sum()
                        for d in range(10))
    if min_digit_acc >= 0.95:
        print(f"  [PASS] All digits ≥ 95% accuracy")
        total_passed += 1
    else:
        print(f"  [FAIL] Some digits below 95%")
        total_failed += 1
    ...
    if after_pool2.shape == (1, 16, 5, 5):
        print(f"  [PASS] Feature map dimensions correct")
        total_passed += 1
    else:
        print(f"  [FAIL] Unexpected feature map shape")
        total_failed += 1
    ...
    print(f"  AND the MLP primitives (GEMM, ReLU) from Exp 001/Study 001")
    ...
    print(f"   Validates Conv2d + MaxPool + FC pipeline end-to-end")
    print(f"\n2. CNN = Conv2d(≈im2col+GEMM) + Pool + FC(=GEMM)")
    print(f"   All ops decompose to GEMM at the primitive level")
    print(f"   BarraCUDA's conv2d.wgsl wraps im2col + gemm_f64")
    print(f"\n3. Vision pipeline validated for BarraCUDA evolution:")
    print(f"   conv2d.wgsl, max_pool2d.wgsl, batch_norm.wgsl")
    print(f"   Same primitives used in ResNet, ViT patch embedding")
