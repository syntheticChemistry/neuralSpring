...
## Architecture (Eukaryotic — post-Wave 155 Aug 2026)

- **UniBin**: Single `neuralspring_unibin` binary with `certify`, `validate`, `serve`, `status`, `version` subcommands
- **IPC tree**: `src/ipc/` — modular facade (`mod.rs`, `client.rs`, `router.rs`, `health.rs`) + 7 per-primal modules (`barracuda`, `toadstool`, `beardog`, `squirrel`, `coralreef`, `skunkbat`, `nestgate`); capability-based discovery with name-hint fallback (`CAPABILITY_HINTS`)
- **Weight loader**: `src/weight_loader/` — `mod.rs` + `safetensors.rs` + `nestgate.rs` + `dtype.rs`
- **Composition validation**: `src/validation/composition.rs` re-exports `discovery.rs`, `json_rpc.rs`, `proto_nucleate.rs`
...
- **Dependencies:** `barracuda` (math engine, `optional = true`, IPC-first), `wgpu` **28** (GPU), `tokio` (async), `tarpc` (optional RPC), `thiserror` (typed errors, all error types; pollster removed). 11 modules feature-gated behind `barracuda`.
...
`surrogate`, `transformer`, spectral analysis, Anderson localization, `coral_forge` (protein folding), `streaming` (FASTA / FASTQ / VCF), GPU dispatch, visualization (`petalTongue`), `weight_spectral` (baseCamp), `weight_loader` (safetensors + NestGate), `certification` (guidestone layers), `validation::composition` (discovery, JSON-RPC, proto-nucleate), `validation::scenarios` (absorbed validators), `ipc` (modular per-primal IPC tree).

## Key Capabilities (JSON-RPC)

Forty-eight capabilities (`domain.verb`) when composed in biomeOS:
...
- **Routing (8):** `primal.forward`, `primal.discover`, `capability.list`, `identity.get`, `mcp.tools.list`, `compute.dispatch`, `compute.dispatch.submit`, `compute.offload`
...
| **Deployment status** | Operational — 13/13 VPS-ready, UDS-first, capability-routed (Wave 155) |
