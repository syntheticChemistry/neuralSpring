...
**Date**: March 15, 2026 (Session 152 — Deep Debt Execution: Tolerance Centralization, Capability Discovery, Shared Validation Infrastructure)
...
**S152**: Deep debt execution — 15+ hardcoded tolerance literals centralized to named `tolerances::` constants (added `IPR_CROSS_PYTHON`). `PrimalClient` evolved to capability-based discovery. coralReef bridge restructured for capability-first discovery. `BIOMEOS_SOCKET_SUBDIR` constant. Shared `validate_tensor_binary` + `gen_test_f64` infrastructure. V103 handoff.
