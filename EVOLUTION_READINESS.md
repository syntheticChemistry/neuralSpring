...
**neuralSpring**: 1,234 lib + 73 forge + 80 playGround tests, 269 binaries, 520+ `.rs` files, 68 modules, 0 clippy warnings (pedantic+nursery+cast deny, workspace-wide), 0 fmt diffs, 0 doctest failures. All 3 crate roots `forbid(unsafe_code)`. All files ≤1000 LOC (largest: 879 LOC). AGPL-3.0-or-later. **Zero C dependencies** (ecoBin compliant). **Rust Edition 2024**. proptest 14 invariants. **Zero `#[allow()]` workspace-wide**. ecoBin CI (musl+ARM cross-compile). `PROVENANCE_REGISTRY` (49 records, 12 tests). `OnceLock` GPU probe cache. Cast lint deny. `PRIMAL_DOMAIN`. `ValidationSink` pattern (5 sinks, 12 tests). 4 provenance integrity tests. IPC resilience wired (RetryPolicy+CircuitBreaker in live PetalTongue path). **30 capabilities** registered (ALL_CAPABILITIES). Deploy graph V137 aligned to `downstream_manifest::neuralspring`. Full deployment standard: health triad, identity.get, mcp.tools.list. Squirrel inference routing wired (try_squirrel_route fallback). Tower Atomic discovery at startup (BearDog + Songbird probes). **4-tier validation**: Python→Rust (Tier 1) → GPU parity (Tier 2) → IPC composition parity (Tier 3) → guideStone bare (Level 3). **guideStone Level 3**: `neuralspring_guidestone` v0.3.0 — 29/29 bare ALL PASS (P1-P5 certified). BLAKE3 CHECKSUMS (15 files) via `primalspring::checksums`. `is_skip_error` unified skip classification. guideStone standard v1.2.0. `IpcMathClient` + `validate_proto_nucleate_capabilities` retained as validation window. primalSpring v0.9.17+. genomeBin v5.1 (46 binaries, 6 target triples). Stadial `deny.toml` bans enforced. 18 barraCuda surface gaps confirmed still open. **Phase 46 composition explorer**: `neural_composition.sh` — agent-driven composition + AI feedback loops (Squirrel-mediated inference, DAG provenance, braid audit trail, closed-loop feedback). Phase 45c BTSP default auto-absorbed.
