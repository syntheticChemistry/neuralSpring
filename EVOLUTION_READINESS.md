...
**Date**: May 13, 2026 (Session S203 — Tier 2 convergence: `barracuda.precision.route` wired (was blocked, now 649 tests upstream). `PrecisionRouteResult` typed response. 7 stale clippy `#[expect]` annotations pruned. exp094 `serde_json` workspace-inherited. LTEE B1 `tolerances.toml`. barraCuda v0.4.0. 731 lib + 11 integration + 73 forge + 80 playGround + 12 exp094 = 907 workspace tests (IPC-first) + 19 certification. V155 handoff)
**barraCuda**: v0.4.0 (`../../primals/barraCuda/crates/barracuda`). 806+ WGSL shaders, wgpu 28, Sprint 2 APIs (activations, rng, tridiagonal_ql), healthSpring domain, batched logsumexp, CoralReefDevice. `PrecisionRoutingAdvice` with `F64NativeNoSharedMem` Ada Lovelace reclassification, `WORKGROUP_SIZE_1D` constant, cross-spring provenance registry, `BatchedOdeRK45F64`, `mean_variance_to_buffer`, coralReef Phase 10 IPC. Three-tier precision: F32/F64/Df64 (lean 3-tier model, F16+templates removed). Deep debt: typed errors, named constants, `Arc<str>` hot-path, `RwLock` compiler, ring buffer back-off, streaming pipeline completion.
...
**neuralSpring**: 731 lib + 11 integration + 73 forge + 80 playGround + 12 exp094 = 907 workspace tests (IPC-first) + 19 certification (guidestone L5), 269 binaries (241 require-features gated), 521+ `.rs` files, 68 modules, 0 clippy warnings (pedantic+nursery+cast deny, workspace-wide), 0 fmt diffs, 0 doctest failures, 0 unfulfilled lint expectations. All 3 crate roots `forbid(unsafe_code)`. All files ≤800 LOC. AGPL-3.0-or-later. **Zero C dependencies** (ecoBin compliant). **Rust Edition 2024**. proptest 24 invariants. **Zero `#[allow()]` workspace-wide**. Zero production mocks. Zero production panics. Zero TODO/FIXME/HACK. ecoBin CI (musl+ARM cross-compile). `PROVENANCE_REGISTRY` (49 records, 12 tests). `OnceLock` GPU probe cache. Cast lint deny. `PRIMAL_DOMAIN`. `ValidationSink` pattern (5 sinks, 12 tests). 4 provenance integrity tests. IPC resilience wired (RetryPolicy+CircuitBreaker in live PetalTongue path). **37 capabilities** registered (ALL_CAPABILITIES) — composition.status + method.register + security.audit_log + toadstool.validate + toadstool.list_workloads + barracuda.precision.route absorbed. Deploy graph V155 aligned to `downstream_manifest::neuralspring`. Full deployment standard: health triad, identity.get, mcp.tools.list. Squirrel inference routing wired (try_squirrel_route fallback). Tower Atomic discovery at startup (BearDog + Songbird probes). **4-tier validation**: Python→Rust (Tier 1) → GPU parity (Tier 2) → IPC composition parity (Tier 3) → guideStone bare (Level 3). **guideStone Level 5**: `neuralspring_guidestone` v0.3.0 — 19 certification tests ALL PASS (L0-L5). BLAKE3 CHECKSUMS via `primalspring::checksums`. `is_skip_error` unified skip classification. guideStone standard v1.2.0. `CapabilityRouter` (20 hints) + `IpcMathClient` + `validate_proto_nucleate_capabilities` retained as validation window. primalSpring v0.9.25+. genomeBin v5.1 (46 binaries, 6 target triples). Stadial `deny.toml` bans enforced. Gap 11 barraCuda surface RESOLVED (S201b). NestGate IPC wired (S202b). Tier 2 COMPLETE (S203): toadStool + barraCuda precision.route all wired. Deep debt all-clear (S203b). **Eukaryotic UniBin** + IPC tree (7 per-primal modules: barracuda, toadstool, beardog, squirrel, coralreef, skunkbat, nestgate) + certification organelle (4 layers) + validation scenarios (6) + 3 fossilized patterns. CI cross-sync 413 methods. skunkBat JH-5 wired. barraCuda v0.4.0.
**S186**: Phase 46 composition explorer — `neural_composition.sh` implements neuralSpring's assigned lane (agent-driven composition + AI feedback loops). Copied `nucleus_composition_lib.sh` (41 functions), `composition_template.sh`, `composition_nucleus.sh` from primalSpring. Squirrel-mediated `inference.complete`/`inference.embed`, DAG branching for AI decisions, braid provenance audit trail (`application/x-neuralspring-agent`), closed-loop feedback (`domain_on_tick` + `check_proprioception`). PRIMAL_GAPS Gap 14 (composition findings). Phase 45c BTSP default auto-absorbed via path dep. V137 handoff.
**S181**: Full composition evolution — 30-capability surface (`health.check`, `identity.get`, `mcp.tools.list` added to `ALL_CAPABILITIES` + niche + capability registry + MCP tools), Squirrel inference routing (`try_squirrel_route` dynamic fallback for `inference.complete`/`embed`/`models`), Tower Atomic startup discovery (`tower.rs` — BearDog + Songbird liveness probes), Tier 3 `validate_composition_evolution` (science→primal→NUCLEUS coherence validator: capability surface completeness, deploy graph alignment, proto-nucleate IPC wiring, inference evolution, health triad), `composed` feature gate, ToadStool discovery fix (`compute.dispatch.submit`), tolerance forensics (`check_abs_or_rel` mode accuracy), deploy graph V131/S181. 1,230+ lib tests, 266 binaries, 520 `.rs` files. V132 handoff.
...
**S173**: Typed spring-level errors (`thiserror`), `nucleus_pipeline` / module decomposition, stale doc reference cleanup. V123 handoff.
...
|-------|---------|------|----------------|
| `barracuda` | path (v0.3.12) | GPU compute abstraction (in-house) | Standalone barraCuda primal (extracted from ToadStool S89) |
