...
**Last reviewed:** barraCuda v0.3.3 at `83aa08a` (`../barraCuda/crates/barracuda`), Mar 10, 2026. wgpu 28, ToadStool S142 (`a86bc546`), coralReef Iteration 29 (`2779c88`). **ALL 17 shortcomings RESOLVED, 46 upstream rewires, 128+ barracuda import sites, 45+ submodules exercised, 233 binaries, 1048 lib tests, 71 forge tests, 0 clippy, 0 doc warnings.** S142: upstream rewire + `enable f64;` PTXAS fix + cross-spring evolution. 220/220 validate_all, 55/55 dispatch parity. V95 handoff.
**Canonical handoff:** V95 S142 (Mar 10, 2026) — `enable f64;` fix + cross-spring evolution + upstream rewire. See `wateringHole/handoffs/NEURALSPRING_V95_S142_TOADSTOOL_BARRACUDA_EVOLUTION_HANDOFF_MAR10_2026.md`
**barraCuda standalone:** `../barraCuda/crates/barracuda` v0.3.3 at `83aa08a` — Sprint 2 APIs (activations, rng, tridiagonal_ql), healthSpring domain, batched logsumexp, CoralReefDevice. 719 WGSL shaders, dual-protocol IPC, domain feature-gates, `PrecisionRoutingAdvice`, cross-spring provenance registry. Extracted from `ToadStool` S89. Deep debt: typed errors, named constants, lint compliance.
**ToadStool:** S142 at `a86bc546` — hardware testing, PCIe transport, ResourceOrchestrator, 19,900+ tests. Hardware discovery + orchestration, coralReef shader proxy, JSON-RPC only (REST removed S90), `SubstrateType` 8 variants, capability-based discovery. Spring sync confirms zero API breakage. Deep debt, zero-copy.
**coralReef:** Iteration 29 at `2779c88` — NVIDIA last mile pipeline, SSA repair, multi-GPU sovereignty, 1200+ tests. Sovereign shader compiler (WGSL → native GPU binary), NVIDIA SM70-SM89, AMD RDNA2+ (E2E GPU dispatch verified), f64 transcendentals.
