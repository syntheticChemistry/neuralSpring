...
**Last reviewed:** barraCuda v0.3.3 standalone (`../barraCuda/crates/barracuda`), Mar 7, 2026. wgpu 28, ToadStool S130 pin, coralReef Iteration 7 pin. **ALL 17 shortcomings RESOLVED, 46 upstream rewires, 128+ barracuda import sites, 45+ submodules exercised, 240 binaries, 883 lib tests, 0 clippy, 0 doc warnings.** S130 sync: `PrecisionRoutingAdvice` wired, `shaders::provenance` available, coralReef renamed from coralNAK, fused GPU test regression gated via canary. 218/218 validate_all.
**Canonical handoff:** V88 (Session 130, Mar 7, 2026) — Upstream rewire + revalidation
**barraCuda standalone:** `../barraCuda/crates/barracuda` v0.3.3 at `2a6c072` — 708 WGSL shaders, dual-protocol IPC, domain feature-gates, `PrecisionRoutingAdvice`, cross-spring provenance registry. Extracted from `ToadStool` S89.
**ToadStool:** S130 at `88a545df` — hardware discovery + orchestration, coralReef shader proxy, JSON-RPC only (REST removed S90), `SubstrateType` 8 variants, capability-based discovery.
**coralReef:** Iteration 7 at `72e6d13` — sovereign shader compiler (WGSL → native GPU binary), NVIDIA SM70-SM89, AMD RDNA2+, f64 transcendentals.
