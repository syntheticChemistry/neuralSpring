...
**Last reviewed:** barraCuda v0.3.7 at `0649cd0` (`../barraCuda/crates/barracuda`), Mar 24, 2026. wgpu 28, ToadStool S146+, coralReef Phase 10. **ALL 17 shortcomings RESOLVED, 25 absorbed workloads, 219 barracuda import sites, 45+ submodules exercised, 261 binaries, ~1,400 tests (1,211 lib + 73 forge + 80 playGround + 9 integration + 25 tokio), 0 clippy, 0 doc warnings.** S152: Deep debt execution — tolerance centralization, capability-based discovery. V103 handoff.
**Canonical handoff:** V103 S152 (Mar 15, 2026) — Deep debt execution: tolerance centralization, capability-based discovery, shared infrastructure. See `wateringHole/handoffs/NEURALSPRING_V103_S152_DEEP_DEBT_BARRACUDA_TOADSTOOL_HANDOFF_MAR15_2026.md`
**barraCuda standalone:** `../barraCuda/crates/barracuda` v0.3.7 at `0649cd0` — `ReduceScalarPipeline` f64 fix, `BatchedComputeDispatch`, `CoralReefDevice`, `FmaPolicy`, `tridiag_eigh_gpu`, 36 tolerances. 719+ WGSL shaders, dual-protocol IPC, domain feature-gates, `PrecisionRoutingAdvice`, cross-spring provenance registry. Extracted from `ToadStool` S89. Deep debt: typed errors, named constants, lint compliance.
**ToadStool:** S146 at `751b3849` — `nvvm_transcendental_risk` in `gpu.info`, PrecisionBrain in `compile_wgsl_multi`, VRAM-aware routing, 19 SpringDomain variants. Hardware discovery + orchestration, coralReef shader proxy, JSON-RPC only (REST removed S90), `SubstrateType` 8 variants, capability-based discovery. Spring sync confirms zero API breakage. Deep debt, zero-copy. 19,900+ tests.
**coralReef:** Iteration 33 at `b783217` — NVVM poisoning fix, struct ABI fixes, 46/46 sovereign compile. Sovereign shader compiler (WGSL → native GPU binary), NVIDIA SM70-SM89, AMD RDNA2+ (E2E GPU dispatch verified), f64 transcendentals. 1608 tests.
**Session 133 sync (Phase 5–7 buildout):** metalForge PCIe `transfer_buffer_strategy()` (P2P vs CPU-staged selection), `NpuToGpuP2P` substrate variant, `mixed_substrate_p2p()`. biomeOS pipeline DAG: `graph.rs` with `StageNode`, `PipelineGraph` (Kahn's topo sort), `PipelineExecution`, 3 canonical pipelines. petalTongue `StreamSession` with backpressure + `push_replace` + `query_capabilities` + 64KB IPC. Feature-gated `validate_all` (`FEATURE_BINARIES`). 3 new validators: `validate_biomeos_graph` 32/32, `validate_petaltongue_scenarios` 31/31, `validate_nucleus_tower` 22/22. 1048 lib + 71 forge tests. 233 binaries. 220/220 validate_all. V91 handoff. **Absorption targets**: `PipelineGraph` → `toadstool::orchestration`, `TransferStrategy` → `barracuda::unified_hardware`, `StreamSession` → `petaltongue-client` crate.
...
| HMM backward (f64) | `ops::bio::hmm::hmm_backward()` | GPU ComputeDispatch f64 backward pass | Available (v0.3.3) |
| HMM Viterbi (f64) | `ops::bio::hmm::hmm_viterbi()` | GPU ComputeDispatch f64 Viterbi decoding | Available (v0.3.3) |
| HMM forward log (f32/f64) | `ops::bio::hmm::{HmmForwardLogF32, HmmForwardLogF64}` | Step-wise log-domain HMM forward | Available (v0.3.3) |
