...
## Kokkos GPU Parity Baseline (External)

**Status**: Gap — referenced but not present in neuralSpring.

`EVOLUTION_READINESS.md` references a Kokkos ↔ BarraCUDA GPU comparison
document at `wateringHole/BARRACUDA_KOKKOS_GPU_BENCHMARK_RESULTS_MAR04_2026.md`,
but this file does not exist in the repository. Kokkos benchmarks live
externally in wateringHole as handoff documents per the
`BARRACUDA_KOKKOS_VALIDATION_BASELINE_NOTICE` convention.

### What Exists

| Baseline | Location | Status |
|----------|----------|--------|
| Python/NumPy vs BarraCUDA CPU | neuralSpring `control/*/bench_*.py` (15 scripts) | Complete |
| BarraCUDA CPU vs GPU | neuralSpring `validate_cpu_gpu_parity` (17 checks) | Complete |
| Kokkos CUDA vs BarraCUDA WGSL | wateringHole (external) | **Not present** |

### Gap Remediation

To close this gap, Kokkos comparison data needs to be:
1. Generated by the BarraCUDA team using representative workloads
2. Published to `wateringHole/` as a handoff document
3. Referenced from `EVOLUTION_READINESS.md` once the file exists

neuralSpring does not need to run Kokkos code — it is an external
Tier 1 performance baseline for the BarraCUDA GPU stack.
