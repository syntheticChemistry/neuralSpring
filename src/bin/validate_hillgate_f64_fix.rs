...
/// The core fix: replace native `pow(` with `pow_f64(` in the shader source.
///
/// `compile_shader_f64` → `inject_missing_math_f64` will then auto-inject
/// the `pow_f64` polyfill from `math_f64.wgsl` (which uses
/// `exp_f64(exponent * log_f64(base))` internally).
