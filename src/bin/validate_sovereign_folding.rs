...
//! nF-01 Phase B: Sovereign folding Evoformer primitive validation.
//!
...
//! ## Provenance
//!
//! Python baseline: `control/sovereign_folding/evoformer_primitives.py`
...
const BASELINE_JSON: &str =
    include_str!("../../control/sovereign_folding/evoformer_baselines.json");
...
fn main() {
    let mut h = ValidationHarness::new("sovereign_folding");
    ...
    let gelu_rust = sovereign_folding::gelu_vec(&gelu_in);
    ...
    let ln_rust = sovereign_folding::layer_norm(&ln_in, n_res, hidden_dim, &gamma, &beta, 1e-5);
    ...
    let sm_rows = 4;
    let sm_cols = 8;
    let sm_rust = sovereign_folding::softmax_rows(&sm_in, sm_rows, sm_cols);
    ...
    let scores_rust =
        sovereign_folding::sdpa_scores(&sdpa_q, &sdpa_k, 1, n_heads, n_res, n_res, head_dim);
    ...
    let sdpa_rust = sovereign_folding::sdpa_full(
    ...
    let tri_out_rust =
        sovereign_folding::triangle_mul_outgoing(&tri_out_a, &tri_out_b, n_res, channels);
    ...
    let tri_in_rust =
        sovereign_folding::triangle_mul_incoming(&tri_in_a, &tri_in_b, n_res, channels);
    ...
    let tri_attn_rust = sovereign_folding::triangle_attention_scores(
        &tri_attn_q,
        &tri_attn_k,
        &tri_attn_bias,
        n_res,
    ...
    let opm_rust =
        sovereign_folding::outer_product_mean(&opm_a, &opm_b, opm_n_seq, n_res, opm_c_a, opm_c_b);
    ...
    let msa_row_scores_rust = sovereign_folding::msa_row_attention_scores(
        &msa_row_q,
        &msa_row_k,
        &msa_row_bias,
        msa_n_seq,
    ...
    let msa_row_out_rust = sovereign_folding::msa_row_attention(
        &msa_row_q,
        &msa_row_k,
        &msa_row_v,
        &msa_row_bias,
    ...
    let msa_col_scores_rust = sovereign_folding::msa_col_attention_scores(
    ...
    let msa_col_out_rust = sovereign_folding::msa_col_attention(
    ...
    let bb_updated = structure_module::backbone_update(&bb_dq, &bb_dt, &ipa_frames, ipa_n);
