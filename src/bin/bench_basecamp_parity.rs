...
async fn main() {
    ...
    println!(
        "  Sub-05 agent_coord     : {cpu_us:8.1} µs  λ₂={:.6}",
        decomp.eigenvalues.get(1).unwrap_or(&0.0)
    );
    ...
    if let Some(ref d) = dev {
        ...
        let corr_op = barracuda::ops::correlation_f64_wgsl::CorrelationF64::new(d.clone()).unwrap();
        ...
        let entropy_op =
            barracuda::ops::fused_map_reduce_f64::FusedMapReduceF64::new(d.clone()).unwrap();
        let probs: Vec<f64> = {
        ...
        let gpu_entropy = entropy_op.shannon_entropy(&probs).unwrap();
