...
async fn main() {
    ...
    expanded::validate_multi_obj_fitness(&mut h, &gpu_disp, &cpu_disp);
    expanded::validate_swarm_nn_forward(&mut h, &gpu_disp, &cpu_disp);
    expanded::validate_integrate_ode_batch(&mut h, &gpu_disp, &cpu_disp);
    expanded::validate_inter_pop_af_variance(&mut h, &gpu_disp, &cpu_disp);
    expanded::validate_hmm_backward_step(&mut h, &gpu_disp, &cpu_disp);
    expanded::validate_hmm_viterbi_step(&mut h, &gpu_disp, &cpu_disp);
    expanded::validate_hmm_chain(&mut h, &gpu_disp, &cpu_disp);
    expanded::validate_detect_introgression(&mut h, &gpu_disp, &cpu_disp);

    // S127: Paper 026 glucose domain — dispatched stats on CGM-scale data
    expanded::validate_glucose_variance_pearson(&mut h, &gpu_disp, &cpu_disp);
