...
pub fn cosine_beta_schedule(t_steps: usize, s: f64) -> NoiseSchedule {
    ...
    let (betas, alpha_bar): (Vec<f64>, Vec<f64>) = f_vals
        .windows(2)
        .map(|w| {
            let ab = (w[1] / f0).clamp(1e-10, 1.0);
            let ab_prev = (w[0] / f0).clamp(1e-10, 1.0);
            let beta = (1.0 - ab / ab_prev).clamp(1e-6, 0.999);
            (beta, ab)
        })
        .unzip();
...
#[must_use]
pub fn forward_diffusion(coords: &[f64], noise: &[f64], t: usize, schedule: &NoiseSchedule) -> Vec<f64> {
...
pub fn ddim_reverse_step(
    ...
    let a_bar_t = schedule.alpha_bar[t];
    let a_bar_prev = if t > 0 { schedule.alpha_bar[t - 1] } else { 1.0 };
...
mod tests {
    ...
    fn cosine_schedule_monotone() {
        ...
        for w in sched.alpha_bar.windows(2) {
            assert!(w[1] <= w[0] + 1e-15, "alpha_bar not monotonically decreasing");
        }
